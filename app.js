...
// ---------------- config (salvata sul telefono, mai su un server) ----------------

function getConfig() {
  try {
    return JSON.parse(localStorage.getItem('qgest_config') || 'null');
  } catch { return null; }
}
function setConfig(cfg) {
  localStorage.setItem('qgest_config', JSON.stringify(cfg));
}

function openSettings(isFirstRun) {
  const cfg = getConfig();
  els.inputUrl.value = cfg?.url || '';
  els.inputToken.value = cfg?.token || '';
  els.settings.classList.remove('hidden');
  els.cancelSettings.classList.toggle('hidden', !!isFirstRun);
}
...
// ---------------- fetch (via JSONP: Safari su iPhone blocca il redirect
// interno che Google fa da script.google.com a googleusercontent.com quando
// la richiesta parte da fetch(); caricare i dati come <script> lo aggira) ----------------

let jsonpCounter = 0;

function jsonpRequest(baseUrl, params, timeoutMs) {
  return new Promise((resolve, reject) => {
    const callbackName = 'qgestCallback_' + (jsonpCounter++) + '_' + Date.now();
...
async function fetchData() {
  const cfg = getConfig();
  if (!cfg) { openSettings(true); return; }

  try {
    const data = await jsonpRequest(cfg.url, { token: cfg.token });
    if (data && data.error === 'unauthorized') {
      showState('error', 'Token non valido', 'Controlla che il token inserito qui corrisponda a quello scritto in CONFIG.token nello script.', true);
      return;
    }
    lastData = data;
    render(data);
